...
// ファイルパス: src/test/java/com/toyota/tsc/notificationhub/commons/JsapUtilTest.java
...
class JsapUtilTest {
    ...
    void executeGetUserId_001() {
        ...
        try (MockedConstruction<RestTemplate> mocked = mockConstruction(RestTemplate.class,
            ...
            verify(rt, times(1)).exchange(eq("https://example/jsap/userid"), eq(HttpMethod.POST),
    ...
    void executeGetUserId_002() {
        ...
        try (MockedConstruction<RestTemplate> mocked = mockConstruction(RestTemplate.class,
                (mock, context) -> when(mock.exchange(anyString(), any(), any(HttpEntity.class), eq(String.class)))
    ...
    void executeDvcLink_001() {
        ...
        try (MockedConstruction<RestTemplate> mocked = mockConstruction(RestTemplate.class,
            ...
            RestTemplate rt = mocked.constructed().get(0);
            ArgumentCaptor<HttpEntity> entityCaptor = ArgumentCaptor.forClass(HttpEntity.class);
            verify(rt, times(1)).exchange(eq("https://example/jsap/dvclink"), eq(HttpMethod.POST),
                    entityCaptor.capture(), eq(String.class));
    ...
    void executePushRequest_001() {
        ...
        try (MockedConstruction<RestTemplate> mocked = mockConstruction(RestTemplate.class,
            ...
            // Act
            ResponseEntity<String> actual = sut.executePushRequest("user-1", "{\"a\":1}", "AUTH");
            ...
            RestTemplate rt = mocked.constructed().get(0);
            ArgumentCaptor<HttpEntity> entityCaptor = ArgumentCaptor.forClass(HttpEntity.class);
            verify(rt, times(1)).exchange(eq("https://example/jsap/notify"), eq(HttpMethod.POST),
                    entityCaptor.capture(), eq(String.class));
    ...
    void executeSendMessage_001() {
        ...
        try (MockedConstruction<RestTemplate> mocked = mockConstruction(RestTemplate.class,
                (mock, c) -> when(mock.exchange(eq("https://example/jsap/notify"), eq(HttpMethod.POST),
                        any(HttpEntity.class), eq(String.class)))
                        .thenReturn(expected))) {

            // Act
            ResponseEntity<String> actual = sut.executeSendMessage("proc-1", "user-1", "1", "title", context, "AUTH");
    ...
    void executeSendMessage_002() {
        ...
        try (MockedConstruction<RestTemplate> mocked = mockConstruction(RestTemplate.class,
                (mock, c) -> when(mock.exchange(anyString(), any(), any(HttpEntity.class), eq(String.class)))
                        .thenThrow(new RuntimeException("boom")))) {
    ...
    void executeGetUserInfo_001() {
        ...
        try (MockedConstruction<RestTemplate> mocked = mockConstruction(RestTemplate.class,
                (mock, c) -> when(mock.exchange(eq("https://example/jsap/userinfo"), eq(HttpMethod.POST),
                        any(HttpEntity.class), eq(String.class)))
                        .thenReturn(expected))) {
            ...
            RestTemplate rt = mocked.constructed().get(0);
            ArgumentCaptor<HttpEntity> entityCaptor = ArgumentCaptor.forClass(HttpEntity.class);
            verify(rt, times(1)).exchange(eq("https://example/jsap/userinfo"), eq(HttpMethod.POST),
                    entityCaptor.capture(), eq(String.class));
