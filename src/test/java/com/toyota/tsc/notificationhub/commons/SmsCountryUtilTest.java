...
// // ファイルパス:
// src/test/java/com/toyota/tsc/notificationhub/commons/SmsCountryUtilTest.java
...
// when(propertiesUtil.getSenderIdToyota()).thenReturn("SIDT");
// when(propertiesUtil.getSmsCountryUser()).thenReturn("user");
// // ★ getSmsCountryPass() は message=null で toHexUtf16BE
// 到達時に例外となり参照されないためスタブしない

// SmsCountryUtil sut = new SmsCountryUtil(propertiesUtil);

// // Act
// CustomException ex = assertThrows(CustomException.class, () ->
// sut.createRequest("1", null, "1"));
...
// (mock, ctx) -> when(mock.postForEntity(eq("https://example/sms"), eq(entity),
// eq(String.class)))
// .thenReturn(ok))) {
...
// verify(rt, times(1)).postForEntity(eq("https://example/sms"), eq(entity),
// eq(String.class));
// }
// }
...
// 非2xxの場合にTscSMSExceptionが送出されることを確認するテストケース
// */
// @Test
...
// HttpEntity<String> entity = new HttpEntity<>("payload", new HttpHeaders());
// ResponseEntity<String> ng =
// ResponseEntity.status(HttpStatus.BAD_REQUEST).body("NG");
...
// // Arrange
// Method m = SmsCountryUtil.class.getDeclaredMethod("toHexUtf16BE",
// String.class);
