...
class SendPushServiceImplTest {
        ...
        @BeforeEach
        void setUp() {
                sut = new SendPushServiceImpl(ntfInfoRepository, notificationHubUtil, properties);
        ...
        void sendPush_001() throws Exception {
                ...
                when(notificationHubUtil.postMessage("inst-001", "payload", "0", "1")).thenReturn(outcome);
        ...
        void sendPush_002() {
                ...
                when(ntfInfoRepository.selectAllByInternalUserId("iu-002")).thenReturn(Collections.emptyList()); // emptyで例外
                                                                                                                 // [1](https://nttdatajpprod-my.sharepoint.com/personal/nobuharu_hoshino_bp_jp_nttdata_com/Documents/Microsoft%20Copilot%20Chat%20%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB/SendPushServiceImpl.java)
                ...
                try (MockedStatic<CommonUtil> commonUtil = mockStatic(CommonUtil.class);
                        ...
                        logUtil.when(() -> LogUtil.error(any(), anyString())).thenAnswer(inv -> null);
        ...
        void sendPush_003() {
                ...
                try (MockedStatic<CommonUtil> commonUtil = mockStatic(CommonUtil.class);
                        ...
                        // Act + Assert
                        assertThrows(TscApplicationException.class, () -> sut.sendPush(request, header)); // validateで投げる
                                                                                                          // [1](https://nttdatajpprod-my.sharepoint.com/personal/nobuharu_hoshino_bp_jp_nttdata_com/Documents/Microsoft%20Copilot%20Chat%20%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB/SendPushServiceImpl.java)
                }
        }
        ...
        void sendPush_004() throws Exception {
                ...
                when(notificationHubUtil.postMessage("inst-004", "payload", "0", "1")).thenThrow(ex); // executePostMessageでcatch
                                                                                                      // [1](https://nttdatajpprod-my.sharepoint.com/personal/nobuharu_hoshino_bp_jp_nttdata_com/Documents/Microsoft%20Copilot%20Chat%20%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB/SendPushServiceImpl.java)
                ...
                try (MockedStatic<CommonUtil> commonUtil = mockStatic(CommonUtil.class);
                        ...
                        // Act + Assert
                        assertThrows(TscNotificationHubsException.class, () -> sut.sendPush(request, header)); // 送信例外
                                                                                                               // [1](https://nttdatajpprod-my.sharepoint.com/personal/nobuharu_hoshino_bp_jp_nttdata_com/Documents/Microsoft%20Copilot%20Chat%20%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB/SendPushServiceImpl.java)
                }
        }
        ...
        void sendPush_006() {
                ...
                when(ntfInfoRepository.selectAllByInternalUserId("iu-006")).thenThrow(new RuntimeException(sqlEx));

                try (MockedStatic<ExtractSqlExceptionUtil> exUtil = mockStatic(ExtractSqlExceptionUtil.class);
                                MockedStatic<CommonUtil> commonUtil = mockStatic(CommonUtil.class);
                                MockedStatic<LogUtil> logUtil = mockStatic(LogUtil.class)) {

                        exUtil.when(() -> ExtractSqlExceptionUtil.findSqlException(any())).thenReturn(sqlEx);
                        exUtil.when(() -> ExtractSqlExceptionUtil.isSqlConnectionError(sqlEx)).thenReturn(true);
                        exUtil.when(() -> ExtractSqlExceptionUtil.isSqlOperationError(sqlEx)).thenReturn(false);
                        ...
                        // Act + Assert
                        assertThrows(CustomException.class, () -> sut.sendPush(request, header)); // 接続エラーはCustomException
                                                                                                  // [1](https://nttdatajpprod-my.sharepoint.com/personal/nobuharu_hoshino_bp_jp_nttdata_com/Documents/Microsoft%20Copilot%20Chat%20%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB/SendPushServiceImpl.java)
                }
        }
        ...
        void sendPush_007() {
                ...
                when(ntfInfoRepository.selectAllByInternalUserId("iu-007")).thenThrow(new RuntimeException(sqlEx));
                ...
                try (MockedStatic<ExtractSqlExceptionUtil> exUtil = mockStatic(ExtractSqlExceptionUtil.class);
                        ...
                        exUtil.when(() -> ExtractSqlExceptionUtil.findSqlException(any())).thenReturn(sqlEx);
                        exUtil.when(() -> ExtractSqlExceptionUtil.isSqlConnectionError(sqlEx)).thenReturn(false);
                        exUtil.when(() -> ExtractSqlExceptionUtil.isSqlOperationError(sqlEx)).thenReturn(true);
                        ...
                        // Act + Assert
                        assertThrows(CustomSqlException.class, () -> sut.sendPush(request, header)); // 操作エラーはCustomSqlException
                                                                                                     // [1](https://nttdatajpprod-my.sharepoint.com/personal/nobuharu_hoshino_bp_jp_nttdata_com/Documents/Microsoft%20Copilot%20Chat%20%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB/SendPushServiceImpl.java)
                }
        }
        ...
        /**
         * クラス：SendPushServiceImpl validateRequiredでinternalUserIdがnullの場合の返却を確認するテストケース
         */
        @Test
        ...
        void createPayload_002() {
                ...
                try (MockedStatic<CommonUtil> commonUtil = mockStatic(CommonUtil.class);
                        ...
                        commonUtil.when(() -> CommonUtil.getMessage(anyString(), any())).thenReturn("MSG");
                        logUtil.when(() -> LogUtil.info(any(), anyString())).thenAnswer(inv -> null);
                        ...
                        Throwable target = ((java.lang.reflect.InvocationTargetException) ex.getCause()).getCause();
        ...
        void executePostMessage_001() throws NotificationHubsException {
                ...
                // Act
                NotificationOutcome outcome = (NotificationOutcome) invokePrivate(sut, "executePostMessage",
                                new Class<?>[] { SendPushRequestDto.class, RequestHeaderDto.class,
                                                NtfInfoEntity.class },
                                new Object[] { request, header, device });
                ...
                // Assert
                assertNull(outcome);
                verify(notificationHubUtil, never()).postMessage(anyString(), anyString(), anyString(), anyString());
        }
        ...
        void sendPush_008() throws Exception {
                ...
                when(ex.isTransient()).thenReturn(true);
                when(ex.httpStatusCode()).thenReturn(503);
                when(notificationHubUtil.postMessage("inst-008", "payload", "0", "1")).thenThrow(ex);
        ...
        void sendPush_009() {
                ...
                when(ntfInfoRepository.selectAllByInternalUserId("iu-009")).thenThrow(new RuntimeException(sqlEx));
        ...
        void sendPush_010() {
                ...
                when(ntfInfoRepository.selectAllByInternalUserId("iu-010")).thenThrow(new RuntimeException("boom"));

                try (MockedStatic<ExtractSqlExceptionUtil> exUtil = mockStatic(ExtractSqlExceptionUtil.class);
                                MockedStatic<CommonUtil> commonUtil = mockStatic(CommonUtil.class);
                                MockedStatic<LogUtil> logUtil = mockStatic(LogUtil.class)) {

                        exUtil.when(() -> ExtractSqlExceptionUtil.findSqlException(any())).thenReturn(null); // ★
                                                                                                             // sqlExなし
        ...
        void executePostMessage_002() throws Exception {
                ...
                when(notificationHubUtil.buildFcmV1Payload(body)).thenThrow(new RuntimeException("boom"));
                ...
                try (MockedStatic<CommonUtil> commonUtil = mockStatic(CommonUtil.class);
                        ...
                        verify(notificationHubUtil, never()).postMessage(anyString(), anyString(), anyString(),
                                        anyString());
                }
        }
        ...
        void getLastData_001() {
                ...
                java.util.List<NtfInfoEntity> list = java.util.List.of(older, newer); // わざと逆順に入れる
