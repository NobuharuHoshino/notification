...
class SaSendPushServiceImplTest {
        ...
        void sendPush_001() {
                ...
                when(jsapUtil.executeGetToken()).thenReturn(new GetALJTokenResultDto(true, TOKEN));
                when(jsapUtil.executeGetUserId("iu", "corr"))
                                .thenReturn(ResponseEntity.ok("{\"resultCode\":\"00001548B123\",\"userId\":\"u\"}"));
                when(jsapUtil.executePushRequest("u", "payload", TOKEN))
        ...
        /**
         * クラス：SaSendPushServiceImpl
         * 異常系（ユーザデータ未取得）でTscApplicationExceptionとなることを確認するテストケース
         */
        @Test
        void sendPush_003() {
                // Arrange
        ...
        /**
         * クラス：SaSendPushServiceImpl
         * 異常系（payload生成例外）でTscApplicationExceptionとなることを確認するテストケース
         */
        @Test
        void sendPush_004() {
                // Arrange
                ...
                // buildFcmV1Payload throws → createPayload catches and throws
                // TscApplicationException
        ...
        void sendPush_009() {
                ...
                // executeGetToken throws RuntimeException（SQLなし）→ catch(Exception) → else →
                // CustomException
        ...
        /**
         * クラス：SaSendPushServiceImpl
         * 異常系（executeGetTokenがnullを返す）でTscApplicationExceptionとなることを確認するテストケース
         */
        @Test
        void sendPush_013() {
                // Arrange
                ...
                // resToken == null → TscApplicationException
        ...
        // ---------------------------------------------------------------------
        // operationPostMessage（reflection）：invalid JSON → JsonProcessingException → CustomException
        // ---------------------------------------------------------------------

        /**
         * クラス：SaSendPushServiceImpl operationPostMessage
         * getUserIdレスポンスが不正JSONの場合にJsonProcessingExceptionが発生しCustomExceptionとなることを確認するテストケース
         */
        @Test
        void operationPostMessage_001() throws Exception {
                // Arrange
                SaSendPushServiceImpl sut = new SaSendPushServiceImpl(saNtfInfoRepository,
                                notificationHubUtil, jsapUtil);
                Method m = SaSendPushServiceImpl.class.getDeclaredMethod(
                                "operationPostMessage",
                                SendPushRequestDto.class, RequestHeaderDto.class, SaNtfInfoEntity.class,
                ...
                // 不正JSONを返す → mapper.readValue がJsonMappingException（JsonProcessingExceptionのサブクラス）をスロー
                when(jsapUtil.executeGetUserId("iu", "corr"))
                ...
                assertThrows(CustomException.class, () -> {
                        try {
                                m.invoke(sut, request, header, userData, "payload", TOKEN);
                        } catch (InvocationTargetException ex) {
                                throw (RuntimeException) ex.getCause();
                        }
                });
