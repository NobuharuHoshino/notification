...
class SaSendPrimaryContactServiceImplTest {
    ...
    void sendPrimaryContact_001() {
        ...
        // JSAP: getToken（成功JSON）
        when(jsapUtil.executeGetToken())
                .thenReturn(ResponseEntity.ok("{\"access_token\":\"test-token\"}"));
        ...
        when(jsapUtil.executeSendMessage(
                eq("proc-001"), eq("u-001"), eq("0"), eq("title"), any(), anyString()))
    ...
    /**
     * クラス：SaSendPrimaryContactServiceImpl
     * 異常系（getUserInfoが2xx以外）でTscPrimaryContactExceptionとなることを確認するテストケース
     */
    @Test
    void sendPrimaryContact_005() {
        ...
        try (MockedStatic<CommonUtil> cu = mockStatic(CommonUtil.class);
            ...
            cu.when(() -> CommonUtil.toJson(any())).thenReturn("{}");
            cu.when(() -> CommonUtil.getSaMessage(anyString(), org.mockito.Mockito.<Object[]>any()))
                    .thenReturn("msg");
            cu.when(() -> CommonUtil.getResultCode(anyString())).thenReturn("RC_ANY");
            lu.when(() -> LogUtil.error(any(), anyString())).thenAnswer(inv -> null);
    ...
    /**
     * クラス：SaSendPrimaryContactServiceImpl
     * 異常系（送信結果NG）でTscPrimaryContactExceptionとなることを確認するテストケース
     */
    @Test
    void sendPrimaryContact_006() {
        ...
        // sendMessage NG
        when(jsapUtil.executeSendMessage(eq("proc-006"), eq("u-006"), eq("0"), eq("title"), any(), anyString()))
    ...
    void sendPrimaryContact_008() {
        ...
        // access_token = null
        when(jsapUtil.executeGetToken())
                .thenReturn(ResponseEntity.ok("{\"access_token\":null}"));
    ...
    void sendPrimaryContact_009() {
        ...
        // access_token = ""
        when(jsapUtil.executeGetToken())
                .thenReturn(ResponseEntity.ok("{\"access_token\":\"\"}"));
